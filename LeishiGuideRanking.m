...
GuideID.NumCysteines = zeros(size(GuideID.GuideSeq));
...
% Step 1: Calculate NumCysteines in the 'Target' column using a 
...
for i = 1:numel(GuideID.GuideTarget)
    ...
    GuideID.NumCysteines(i) = numCsWithinBrackets;
...
GuideID.RelativeProbability = GuideID.NumPossibleStopCodons ./ GuideID.NumCysteines;

% Step 4: Check if 'C' is located in positions 4 to 8
...
scores = (GuideID.RelativeProbability + GuideID.C4to8Score + GuideID.("TargetWithinFirst_20%_of_CDS")+ GuideID.("TargetWithinFirst_40%_of_CDS") + GuideID.("EditWindow_4-10"));

%normalize scores by maximal possible score = 100
normalized_scores = scores/5; %num of contributions

% Add the normalized_scores as a new column 'C_Scores' to the table
GuideID.C_Scores = normalized_scores;
...
for i = 1:size(GuideID, 1)
    ...
    NewIdentifier{i} = [geneID '_' strandPart '_' num2str(cScore)];
end
